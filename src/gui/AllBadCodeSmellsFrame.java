...
public class AllBadCodeSmellsFrame {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					AllBadCodeSmellsFrame window = new AllBadCodeSmellsFrame("dnsjava-2.1.6");
	...
	public AllBadCodeSmellsFrame(String name) {
		initialize(name);
	}
	...
	private void initialize(String name) {
		...
		frame = new JFrame("All bad code smells into  the project");
		 ...
		 * ()); badCodeSmells.addAll(controller.getViolationManagers().get(name).
		 * getLargeClasses());
		 * badCodeSmells.addAll(controller.getViolationManagers().get(name).
		 * getLongMethods());
		 * badCodeSmells.addAll(controller.getViolationManagers().get(name).
		 * getLongParametersLists());
		 * 
		 * for (Duplication d :
		 * controller.getDuplicationManagers().get(name).getDuplications()) {
		...
		ArrayList<BadCodeSmell> badCodeSmells = new ArrayList<BadCodeSmell>(
				controller.getDbManager().selectFromBadCodeSmells(systemName, systemVersion));
